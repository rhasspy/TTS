...
def text2phone(text, language, backend="phonemizer", word_breaks=True):
    '''
    Convert graphemes to phonemes.
    '''
    if backend == "gruut":
        # Use gruut instead of phonemizer
        return text2phone_gruut(text, language, word_breaks=word_breaks)
...
def phoneme_to_sequence(text, cleaner_names, language, enable_eos_bos=False, tp=None,
                        backend="phonemizer", text_is_phonemes=False, word_breaks=True):
    # pylint: disable=global-statement
    ...
    if text_is_phonemes:
        ...
        to_phonemes = text2phone(clean_text, language, backend=backend, word_breaks=word_breaks)
        if not to_phonemes:
