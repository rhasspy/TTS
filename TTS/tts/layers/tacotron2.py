...
from .common_layers import Linear, Prenet, init_attn
...
class Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths):
        ...
        for layer in self.convolutions:
            o = layer(o)
        o = o.transpose(1, 2)
        o = nn.utils.rnn.pack_padded_sequence(o, input_lengths, batch_first=True)
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.attention_rnn = nn.LSTMCell(
            self.prenet_dim + input_dim, self.query_dim, bias=True
        )
        ...
        self.decoder_rnn = nn.LSTMCell(
            self.query_dim + input_dim, self.decoder_rnn_dim, bias=True
        )

        self.linear_projection = Linear(
            self.decoder_rnn_dim + input_dim, self.frame_dim * self.r_init
        )

        self.stopnet = nn.Sequential(
            nn.Dropout(0.1),
            Linear(
                self.decoder_rnn_dim + self.frame_dim * self.r_init,
                1,
                bias=True,
                init_gain="sigmoid",
            ),
    ...
    def get_go_frame(self, inputs):
        B = inputs.size(0)
        memory = torch.zeros(1, device=inputs.device).repeat(B, self.frame_dim * self.r)
        return memory
